...
import { Hammer, Mail, AlertTriangle, CheckCircle, ArrowLeft } from 'lucide-react';
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center bg-[#09090b] overflow-hidden px-4">
      ...
      <div className="relative w-full max-w-md z-10">
        ...
        >
          <h2 className="text-xl font-bold text-white mb-3">Reset Password</h2>
          <p className="text-xs text-zinc-500 mb-6">Enter your registered email address and we'll send you instructions to reset your password.</p>
          ...
          {message && (
            <motion.div 
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
              className="flex items-start gap-2.5 p-3.5 mb-5 rounded-xl bg-emerald-500/5 border border-emerald-500/20 text-emerald-300 text-xs"
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2" htmlFor="forgot-email">
                Manager Email
              </label>
          ...
          <div className="mt-8 text-center">
            <Link href="/login" className="inline-flex items-center gap-1.5 text-xs text-amber-500 hover:text-amber-400 font-bold transition-colors">
